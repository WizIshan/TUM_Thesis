...
For the purposes of this study we have the get_ceat() function which can be called to generate the CEAT scores.
Some small changes were made so that the results generated are not written to files but instead passed as outputs via the get_ceat function.
...
# weat_groups = [[instruments, weapons, pleasant, unpleasant]]
...
def ceat_meta(weat_groups = weat_groups, model='bert', exp_name = 'None', test=1,N=10000, input_dir = None):
    # nm = "ceat_race_{}_vector.pickle".format(model)
    nm = exp_name + "weat{}.pickle".format(test)
    # print(nm)
    weat_dict = pickle.load(open(os.path.join(input_dir,nm),'rb'))
    ...
    e_nm = "{0}_{1}_es.pickle".format(model,test)
    v_nm = "{0}_{1}_v.pickle".format(model,test)
...
def get_ceat(input_dir,output_dir,model,tokenizer, exp_name, lm_model='bert', generate_new = False):
    ...
    for e in range(1,15):
        ...
        result['PES'].append(pes)
        result['p-value'].append(p_value)

    result['group'].append(0)
    result['p-value'].append(0.0)
    result['PES'].append(avg_pes/14)
